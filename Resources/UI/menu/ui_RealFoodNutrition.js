...
function createRealFoodGUI() {

	ti = {
		RealFood : Ti.UI.createScrollView({
			width : '100%',
			height : Ti.UI.FILL,
			left : 0,
			top : 0,
...
function buildRealFoodUI() {
	...
	for (var i = 0, j = APP_DATA.length; i < j; i++) {
		...
		if (APP_DATA[i].cat == "Real Food Nutrition") {
			...
			ti.RealFood.add(container);
		}
	};

}

exports.getRealFoodGUI = function() {
	ti.RealFood.removeAllChildren();
	buildRealFoodUI();
	return ti.RealFood;
};

function eventListenersRealFood() {
	ti.RealFood.addEventListener('click', function(e) {
		if (e.source.header != undefined) {
			App.UI.storiesDetails.getstoriesDetailsComp(e.source);
		}
	});

	Ti.App.addEventListener('SEARCH_REAL', function(e) {
