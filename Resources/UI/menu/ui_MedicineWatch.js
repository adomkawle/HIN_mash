...
function createMedicineWatchGUI() {

	ti = {
		MedicineWatch : Ti.UI.createScrollView({
			width : '100%',
			height : Ti.UI.FILL,
			left : 0,
			top : 0,
...
function buildMedicineWatchUI() {
	...
	for (var i = 0, j = APP_DATA.length; i < j; i++) {
		...
		if (APP_DATA[i].cat == "Medicine Watch") {
			...
			ti.MedicineWatch.add(container);
		}
	};

}

exports.getMedicineWatchGUI = function() {
	ti.MedicineWatch.removeAllChildren();
	buildMedicineWatchUI();
	return ti.MedicineWatch;
};

function eventListenersMedicineWatch() {
	ti.MedicineWatch.addEventListener('click', function(e) {
		if (e.source.header != undefined) {
			App.UI.storiesDetails.getstoriesDetailsComp(e.source);
		}
	});
	
		Ti.App.addEventListener('SEARCH_MED', function(e) {
...
exports.initialize = function(app) {
	App = app;
	createMedicineWatchGUI();
	eventListenersMedicineWatch();
};
